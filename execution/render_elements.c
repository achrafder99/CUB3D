...
/*   Updated: 2023/08/28 15:53:56 by adardour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_ceiling(t_mlx *mlx)
{
	...
	while (i < mlx->win_w)
	{
		draw_project(mlx, i, 0, mlx->rays[i].top_wall, mlx->data->ceiling.color);
		i++;
	}
}
...
void	render_floor(t_mlx *mlx)
{
	...
	while (i < mlx->win_w)
	{
		draw_project(mlx, i, mlx->rays[i].bot_wall, mlx->win_h, mlx->data->floor.color);
		i++;
	}
}
