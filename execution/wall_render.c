...
/*   Updated: 2023/09/11 16:27:10 by adardour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_based_on_direction(t_mlx *mlx, int i, char dir)
{
	if (dir == 'n')
	{
		mlx->text_n.text_offset = (int)fmod(mlx->rays[i].hit_x * (mlx->text_n.w / TILE_SIZE), mlx->text_n.w);
		draw_north_text(mlx, (int)mlx->rays[i].top_wall,
			(int)mlx->rays[i].bot_wall, i);
	}
	else if (dir == 's')
	{
		mlx->text_s.text_offset = (int)fmod(mlx->rays[i].hit_x* (mlx->text_s.w / TILE_SIZE), mlx->text_s.w);
		draw_south_text(mlx, (int)mlx->rays[i].top_wall,
			(int)mlx->rays[i].bot_wall, i);
	}
	else if (dir == 'e')
	{
		mlx->text_e.text_offset = (int)fmod(mlx->rays[i].hit_y* (mlx->text_e.w / TILE_SIZE), mlx->text_e.h);
		draw_east_text(mlx, (int)mlx->rays[i].top_wall,
			(int)mlx->rays[i].bot_wall, i);
	}
	else
	{
		mlx->text_w.text_offset = (int)fmod(mlx->rays[i].hit_y* (mlx->text_w.w / TILE_SIZE), mlx->text_w.h);
